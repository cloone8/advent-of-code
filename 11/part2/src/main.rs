...
impl MonkeyTest {
    ...
    fn get(&self) -> u64 {
        match self {
            MonkeyTest::Div(divisor) => *divisor
        }
    }
}
...
fn parse_monkeys(file: File) -> (Vec<Monkey>, u64) {
    ...
    let monkeys: Vec<Monkey> = monkey_strings.iter()
...
fn do_round(monkeys: &mut Vec<Monkey>, modulo: u64) {
    for i in 0..monkeys.len() {
        let cur_monkey = monkeys[i].clone();

        for item in cur_monkey.items {
            let new_item = cur_monkey.operation.apply(item);

            let (true_monkey, false_monkey) = cur_monkey.test_action;

            if cur_monkey.test.apply(new_item) {
                monkeys[true_monkey].items.push_back(new_item % modulo);
            } else {
                monkeys[false_monkey].items.push_back(new_item % modulo);
            }
        }
...
fn main() {
    ...
    let (mut monkeys, modulo) = parse_monkeys(file);

    for _ in 0..10000 {
        do_round(&mut monkeys, modulo);
    }
