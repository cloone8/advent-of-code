    ...
    for line in input.lines() {
        match line {
            Ok(line) => {
                ...
                if elf0_range.0 >= elf1_range.0 && elf0_range.1 <= elf1_range.1 {
                    // Elf 0 fully in elf 1?
                    num_overlaps += 1;
                } else if elf1_range.0 >= elf0_range.0 && elf1_range.1 <= elf0_range.1 {
                    // Elf 1 fully in elf 0
                    num_overlaps += 1;
                }
            }
...
fn main() {
    let input_file = match File::open("test.txt") {
        Ok(file) => file,
