use std::{fs::File, io::{BufRead, BufReader}, collections::{VecDeque, HashSet}, cmp::{max, min}};
...
fn parse_grid(input: impl BufRead) -> (Vec<Vec<Square>>, (usize, usize), Point) {
...
fn do_search(grid: &Vec<Vec<Square>>, grid_dims: (usize, usize), queue: &mut VecDeque<(Point, u64)>, visited: &mut HashSet<Point>, cur: (Point, u64)) -> Option<u64> {
...
fn main() {
    ...
    let mut queue: VecDeque<(Point, u64)> = vec![(start, 0)].into_iter().collect();
